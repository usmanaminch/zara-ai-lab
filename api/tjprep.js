  ...
  if (mode === 'prompt') {
    const response = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        messages: [{ role: 'user', content: type === 'pse' ? (() => { const mathTopics = ["unit conversions and rate problems", "geometry (area, volume, perimeter of real-world shapes)", "ratios, proportions, and percentages", "number patterns and sequences", "budget and cost problems with multiple items", "mixture problems", "multi-step algebra word problems"]; const scienceTopics = ["genetics and Punnett squares (dominant/recessive traits)", "ecology and population reasoning (food chains or predator-prey)", "environmental science (energy use or water conservation)", "biology reasoning (disease spread rates or population growth)", "basic chemistry reasoning (concentrations or dilutions in plain language)"]; const useScience = Math.random() > 0.4; const topicList = useScience ? scienceTopics : mathTopics; const topic = topicList[Math.floor(Math.random() * topicList.length)]; return `Generate a realistic TJHSST PSE prompt about: ${topic}. Seed: ${Math.random()}. Make it multi-step, solvable without advanced formula knowledge, and include a judgment or opinion question at the end if it fits naturally. Write in a style similar to the real TJ PSE.`; })() : type === 'sps-sim' ? `Generate 4 SPS prompts for a full simulation. Seed: ${Math.random()}. Each must test a different Portrait of a Graduate trait. Return JSON array of 4 objects with fields: prompt and trait.` : (() => { const traits = ['Communicator', 'Collaborator', 'Creative and Critical Thinker', 'Ethical and Global Citizen', 'Goal-Directed and Resilient Individual']; const trait = traits[Math.floor(Math.random() * traits.length)]; return `Generate an SPS prompt specifically testing the trait: ${trait}. Seed: ${Math.random()}. Make the prompt specific and reflective, similar to what TJ actually asks. Do not repeat this exact trait wording in the prompt itself.`; })() }]
      }),
    });
    const data = await response.json();
    let text = data.content?.[0]?.text || '{}';
