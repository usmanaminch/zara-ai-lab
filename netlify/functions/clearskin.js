export default async (request) => {
  ...
  if (!symptom || symptom.trim().length < 3) {
    return new Response(JSON.stringify({ error: "Please describe your symptom" }), { status: 400 });
  }
...
Never be overly judgmental — kindly inform instead.`,
      messages: [{
        role: "user",
        content: `I have this skin concern: ${symptom}`
      }]
    }),
  });
...
export const config = { path: "/api/clearskin" };
